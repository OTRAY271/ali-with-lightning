...
class CelebADiscriminator(BaseDiscriminator):
    def __init__(self, latent_dim: int = 256):
        x_layers = nn.Sequential(
            self._xconv_block(3, 64, kernel_size=2, stride=1),
            self._xconv_block(64, 128, kernel_size=7, stride=2),
            self._xconv_block(128, 256, kernel_size=5, stride=2),
            self._xconv_block(256, 256, kernel_size=7, stride=2),
            self._xconv_block(256, 512, kernel_size=4, stride=1),
        )

        z_layers = nn.Sequential(
            self._conv_block(latent_dim, 512, kernel_size=1, stride=1),
            self._conv_block(512, 512, kernel_size=1, stride=1),
        )
    ...
    def _xconv_block(
        ...
        return super()._conv_block(
            in_channels, out_channels, kernel_size, stride, 0.0, 0.02, True
        )
